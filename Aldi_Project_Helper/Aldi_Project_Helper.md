...
`compareSyncFiles(localFiles, remoteFiles)` decides upload vs download by comparing mtimes — a remote file ≥ local wins (download), otherwise upload. Files matching `shouldSkipFile()` (`.DS_Store`, `Thumbs.db`, etc.) are ignored.

**Date folder matching & "Latest" selection.** A date folder is a name that starts with a `YYMMDD` six-digit date, optionally followed by an appended label that producers add for organization (e.g. `260526` or `260526_MaFo`). The 6 digits must be followed by a non-digit or end-of-name, so `2605261` is *not* a date folder. The "Latest" / "Latest 5" dropdown counts **distinct dates**, not folders: the script picks the latest N distinct `YYMMDD` prefixes and then syncs *every* folder sharing one of those dates. So if `260526`, `260526_MaFo`, and `260526_something_else` all exist, "Latest" syncs all three. This logic is duplicated across the Ae path (`isDateFolder` / `getDatePrefix` in `_ftp.jsx`, selection in `Aldi_Project_Helper_V2.jsx`) and the two CLI helpers (`is_date_folder` in `_ftp_sync.sh`, `Test-DateFolder` / `Get-DatePrefix` in `_ftp_sync.ps1`) — keep all three in sync when changing it.
